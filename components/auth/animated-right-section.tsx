...
export function AnimatedRightSection({ title, subtitle, type }: AnimatedRightSectionProps) {
  ...
  return (
    <div className="hidden lg:flex flex-col justify-between items-center p-12 bg-gradient-to-br from-emerald-600 via-teal-600 to-emerald-700 dark:from-emerald-950 dark:via-teal-950 dark:to-emerald-900 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center flex-1 space-y-8 text-center">
        ...
        >
          <h2 className="text-5xl md:text-6xl font-bold text-white leading-tight text-balance">{title}</h2>
          <p className="text-xl md:text-2xl text-white/80 max-w-lg mx-auto text-balance">{subtitle}</p>
        </div>

        {/* Feature Icons */}
        <div
          className={`flex gap-6 justify-center flex-wrap transform transition-all duration-1000 delay-300 ${
            isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
          }`}
        >
      ...
      {/* Bottom CTA or Stats */}
      <div
        className={`relative z-10 text-center space-y-3 transform transition-all duration-1000 delay-500 ${
          isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
        }`}
      >
        {type === "login" ? (
          <p className="text-white/70 text-sm">Welcome back to your learning platform</p>
        ) : (
          <div className="space-y-2">
            <p className="text-white/90 text-base font-semibold">✨ Get started in less than 2 minutes</p>
