...
2. **Check for existing branches before creating new one**:
   ...
   - Run the script `.specify/scripts/bash/create-new-feature.sh` with the calculated number and short-name:
     ...
     - PowerShell example: `& .specify/scripts/bash/create-new-feature.sh --json "$ARGUMENTS" --number 5 --short-name "user-auth" "Add user authentication"`
...
3. Create and Fill PHR (Shell first; fallback agent‑native)
   ...
   - Read `.specify/templates/phr-template.prompt.md` (or `templates/phr-template.prompt.md`)
     ...
     - Fill placeholders and embed full PROMPT_TEXT and concise RESPONSE_TEXT - Read `.specify/templates/phr-template.prompt.md` (or `templates/…`)
     - Allocate an ID; compute the output path based on stage from step 2; write the file
